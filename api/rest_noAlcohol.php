...
//Creates an instance of the class "NoAlcohol"
$drinks = new NoAlcohol();
...
switch($method) {
    ...
    //And if an id is not present the method "getDrinks" is used to collect all information from the database table
        if(isset($id)){
            $response = $drinks->getDrinkById($id);
        }else{
            $response = $drinks->getDrinks();
        }
    ...
    //If the first if-statement is false the response code will be 400 and the message "All fields must be filled in!" will show.
        if($drinks->setDrink($data['drinkName'], $data['drinkPrice'])){
    ...
    //went wrong      
            if($drinks->addDrink($data['drinkName'], $data['drinkPrice'])){
            $response = array("message" => "Drink has been added");
            http_response_code(201);
            }else{
                $response = array("message" => "Something went wrong, the drink has not been added!");
    ...
    //If the first if-statement is false the response code will be 400 and the message "All fields must be fille in!" will show.
        if($drinks->setDrinkAndId($data['id'], $data['drinkName'], $data['drinkPrice'])){
    ...
    //If something goes wrong the response code will be 500 wich stands for internal error              
            if($drinks->updateDrink($data['id'], $data['drinkName'], $data['drinkPrice'])){
            
                $response = array("message" => "Drink has been updated!");
                http_response_code(200);
            }else{
                $response = array("message" => "Something went wrong, the drink has not been updated");
                http_response_code(500);
            }
        }else{
            $response = array("message" => "All fields must be filled in!");
    ...
    case 'DELETE':
        if(!isset($id)) {
            ...
            if($drinks->deleteDrink($id)){
                http_response_code(200);
                $response = array("message" => "The drink has been deleted!");
            }
        }
        break;
